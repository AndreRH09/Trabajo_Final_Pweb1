...
if(checkOwner($nombre,$marca)){
  my @arr=obtener($nombre,$marca);
  agregarCarrito(@arr[0],@arr[2],@arr[3],@arr[4],@arr[5]);
...
sub agregarCarrito{
  ...
  my $sql = "INSERT INTO carrito(nombre,modelo,precio,marca,codigo) VALUES(?,?,?,?,?)";
  my $sth = $dbh->prepare($sql);
  $sth->execute($nombre,$modelo,$precio,$marca,$codigo);
  $sth->finish;
  $dbh->disconnect;
}
sub checkOwner{
