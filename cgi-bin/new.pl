...
my $modelo=$q->param('modelo');
my $precio=$q->param('precio');
my $marca=$q->param('marca');
my $codigo=$q->param('codigo');
if(defined($nombre) && defined($precio) && defined($stock) && defined($marca)){
  if(checkOwner($nombre,$marca)){
    print renderXML();
  }else{
    newProd($nombre,$stock,$modelo,$precio,$marca,$codigo);
...
sub newProd{
  ...
  my $sql = "INSERT INTO inventario(nombre,stock,modelo,precio,marca,codigo) VALUES(?,?,?,?,?,?)";
  my $sth = $dbh->prepare($sql);
  $sth->execute($nombre,$stock,$modelo,$precio,$marca,$codigo);
  $sth->finish;
  $dbh->disconnect;
}
sub checkOwner{
